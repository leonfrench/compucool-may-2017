...
**You'll need:** A [REDCap account](#preparation) (optional).
...
**Description:** REDCap has a REST API (Application Programming Interface) based on simple HTTP requests, and all REDCap users at CAMH are able to use it (after requesting a REDCap "token"). Let's explore what the API allows us to do together!

Any programming language with a HTTP library can use the REDCap API, but we'll be using Python (via IPython/Jupyter Notebooks) for this workshop.

 **You'll learn:**

- what is an API request payload?
- how to write payloads for data export, import, etc
